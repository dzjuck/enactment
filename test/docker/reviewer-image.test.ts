...
/** Trips `detect-child-process`, an ERROR rule in the vendored set. */
const CRITICAL = `const { exec } = require('child_process');
exec('ls ' + process.argv[2]);
`;

/** Trips `detect-eval-with-expression`, a WARNING rule in the vendored set. */
const WARNING = `function run(input) {
  return eval(input);
}
module.exports = { run };
`;
  ...
  const run = await runContainer(
    {
      ...
      network: "none",
      mounts: [
        { type: "bind", source: dir, target: REVIEW_ROOT, readonly: true },
      ],
    },
...
describe("pinned reviewer image", () => {
    ...
    const result = await runContainer(
      {
        ...
        argv: [
          "sh",
          "-c",
          "id -u; id -g; touch /probe 2>&1 || echo read-only-root",
        ],
        network: "none",
      },
      { timeoutSeconds: 120, graceSeconds: 5 },
  ...
  it("reports an ERROR finding with rule ID, path, positions and matched text", async () => {
    ...
    expect(finding?.check_id).toContain("detect-child-process");
  ...
  it("reports a WARNING finding", async () => {
    const { json } = await scan({ "after/src/eval.js": WARNING }, "warning");

    expect(json.results.map((result) => result.extra.severity)).toEqual([
      "WARNING",
    ]);
  }, 300_000);
  ...
  it("keeps identical findings separate under both prefixes and after a line shift", async () => {
    const { json } = await scan(
      {
        "before/src/eval.js": WARNING,
        "after/src/eval.js": `// a new leading comment\n${WARNING}${WARNING}`,
      },
      "duplicates",
    );

    const byRoot = json.results.filter((result) =>
      result.path.startsWith(REVIEW_AFTER_ROOT),
    );
    ...
    expect(
      new Set(byRoot.map((result) => result.extra.lines.trim())).size,
    ).toBe(1);
