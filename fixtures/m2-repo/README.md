...
Minimal reusable repository and task for a real two-agent RED → GREEN run.
...
node dist/cli.js run "$repo/task.yml" --repo "$repo" --artifacts "$artifacts"
```
...
To repeat the task, run the block again. Do not reuse the completed scratch repository.
