...
const NOT_FOUND = 'Error response from daemon: network enactment-net-1-egress not found';
const IN_USE =
  'Error response from daemon: error while removing network: network enactment-net-1-egress ' +
...
describe('removeNetwork', () => {
  it('removes an existing network', async () => {
    await expect(removeNetwork('enactment-net-1-egress')).resolves.toBeUndefined();

    expect(calls).toEqual([['network', 'rm', 'enactment-net-1-egress']]);
  });
  ...
  it('propagates a real removal failure instead of reporting success', async () => {
    ...
    const failure = await removeNetwork('enactment-net-1-egress').catch(
      (cause: unknown) => cause,
    );

    expect(failure).toBeInstanceOf(NetworkRemovalError);
    expect((failure as Error).message).toContain('enactment-net-1-egress');
  ...
  it('propagates a failure it cannot interpret rather than assuming not-found', async () => {
    ...
    await expect(removeNetwork('enactment-net-1-egress')).rejects.toThrow(NetworkRemovalError);
  });
});
