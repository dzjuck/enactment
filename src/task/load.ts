...
export interface LoadedTask {
  task: Task;
  /** sha256 of the file bytes, so any byte change is a different task. */
  hash: string;
}
...
export async function loadTask(filePath: string): Promise<LoadedTask> {
  ...
  const result = taskSchema.safeParse(document);
  if (!result.success) {
    const details = result.error.issues.map((issue) => `  - ${formatIssue(issue)}`).join('\n');
    throw new TaskConfigError(`Invalid task file ${filePath}:\n${details}`, filePath);
  }

  return { task: result.data, hash };
}
